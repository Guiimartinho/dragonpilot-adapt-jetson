...
![DragonPilot running on Jetson AGX Xavier](docs/jetson/assets/ui_running_on_jetson_3.png)
*DragonPilot driving view on Jetson AGX Xavier — model path, lane lines, lead indicator, and DMS all active at 1920x960 (SCALE=0.889)*

![DragonPilot driving view 2](docs/jetson/assets/ui_running_on_jetson_2.png)
*DragonPilot driving view with lane detection and road camera feed — modeld 13.66ms median, 20 FPS, 0 errors*
...
| UI (raylib) | Working | Centered window, VSync, 60 FPS, model lines rendering |
...
- **Model Inference**: tinygrad `DEV=QCOM` -> `DEV=CUDA` with FP16 + Tensor Cores + BEAM=2
- **Camera**: Qualcomm Spectra ISP -> USB webcam / MIPI CSI-2
- **Memory**: ION allocator -> cudaHostRegisterMapped (GPU-direct zero-copy)
- **Hardware Class**: New `Jetson` class with thermal, power, DFS, and fan management
- **Real-Time Scheduling**: Graceful fallback when SCHED_FIFO not permitted
- **Display**: 2160x1080 (comma 3) -> 1920x960 via SCALE=0.889, centered on 1920x1080 HDMI
...
- [Operational Guide](JETSON_SETUP.md) - How to run on the Jetson (SSH, VNC, replay, benchmark)
- [Optimization Details](JETSON_OPTIMIZATION.md) - All optimizations, benchmarks, bugs, files
- [Setup Guide](docs/jetson/SETUP_GUIDE.md) - Complete step-by-step guide to build and run
- [Jetson Port Overview](docs/jetson/README.md) - Status, architecture, links
...
![DragonPilot rodando na Jetson AGX Xavier](docs/jetson/assets/ui_running_on_jetson_3.png)
*Visao de conducao do DragonPilot na Jetson AGX Xavier — path do modelo, lane lines, indicador de lead e DMS ativos a 1920x960 (SCALE=0.889)*

![DragonPilot visao de conducao 2](docs/jetson/assets/ui_running_on_jetson_2.png)
*Visao de conducao com deteccao de faixa e feed da camera — modeld 13.66ms median, 20 FPS, 0 erros*
...
| CUDA Zero-Copy Preprocessing | Funcionando | 13.66ms median, 0% frame drops, Tensor.from_blob |
...
| UI (raylib) | Funcionando | Janela centralizada, VSync, 60 FPS, linhas do modelo renderizando |
...
- **GPU Compute**: Qualcomm QCOM/Adreno -> NVIDIA CUDA (Volta sm_72)
- **Inferencia de Modelos**: tinygrad `DEV=QCOM` -> `DEV=CUDA` com FP16 + Tensor Cores + BEAM=2
- **Camera**: Qualcomm Spectra ISP -> Webcam USB / MIPI CSI-2
- **Memoria**: Alocador ION -> cudaHostRegisterMapped (GPU-direct zero-copy)
- **Classe Hardware**: Nova classe `Jetson` com gerenciamento termico, energia, DFS e ventilador
- **Scheduling Real-Time**: Fallback gracioso quando SCHED_FIFO nao permitido
- **Display**: 2160x1080 (comma 3) -> 1920x960 via SCALE=0.889, centralizado em HDMI 1920x1080
...
- [Guia Operacional](JETSON_SETUP.md) - Como rodar na Jetson (SSH, VNC, replay, benchmark)
- [Detalhes de Otimizacao](JETSON_OPTIMIZATION.md) - Todas otimizacoes, benchmarks, bugs, arquivos
- [Guia de Setup](docs/jetson/SETUP_GUIDE.md) - Guia completo passo-a-passo para compilar e rodar
- [Visao Geral do Port](docs/jetson/README.md) - Status, arquitetura, links
